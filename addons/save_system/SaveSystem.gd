...
func _ready():
	...
	for property in res.get_property_list():
		if not property.name in base_property_names:
			base_property_names.append(property.name)
	var ref = RefCounted.new()
	for property in ref.get_property_list():
		if not property.name in base_property_names:
			base_property_names.append(property.name)
...
func _resource_to_dict(resource : Resource) -> Dictionary:
	...
	for property in resource.get_property_list():
		if(base_property_names.has(property.name) or property.name.ends_with(".gd")): 
			continue

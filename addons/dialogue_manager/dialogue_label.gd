...
var split_pause_at_characters: Array
var split_pause_at_characters_less_period: Array
...
func _ready() -> void:
	split_pause_at_characters = pause_at_characters.split()
	split_pause_at_characters_less_period = split_pause_at_characters.duplicate()
	split_pause_at_characters_less_period.erase(".")
...
func _type_next(delta: float, seconds_needed: float) -> void:
	...
	if _should_auto_pause():
		additional_waiting_seconds += seconds_per_step * 7.5
	...
	if _last_wait_index != visible_characters and additional_waiting_seconds > 0:
		...
		if visible_characters <= get_total_character_count():
			spoke.emit(parsed_text[visible_characters - 1], visible_characters - 1, _get_speed(visible_characters))
...
#	if (
#		visible_characters > 1
#		and parsed_text[visible_characters - 1] in split_pause_at_characters_less_period
#	):
#		return false

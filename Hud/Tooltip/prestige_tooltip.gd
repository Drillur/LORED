	...
	match menu.type:
		UpgradeMenu.Type.MALIGNANT:
			if gv.stage2.unlocked:
				var t = up.get_upgrade_menu(UpgradeMenu.Type.NORMAL).details.colored_name
				var s = gv.get_stage(Stage.Type.STAGE1).details.colored_name

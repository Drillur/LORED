...
func _init(_type: int, details: Dictionary) -> void:
	...
	if "job" in details.keys():
		job = details["job"]
	if "affected_lored" in details.keys():
		affected_lored = details["affected_lored"]
...
func add_affected_lored(lored_type: int) -> void:
...
func apply() -> void:
	if not applied:
		...
		if replaced_upgrade_type >= 0:
			var replaced_effect = up.get_upgrade(replaced_upgrade_type).effect as OldUpgradeEffect

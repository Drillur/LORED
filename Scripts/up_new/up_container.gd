...
func procedure():
	...
	if gv.r["malig"].isLessThan(gv.up["ROUTINE"].cost["malig"].t):
		return
...
func init() -> void:
	...
	if 5==5:
		...
		var i = 0
		var stage = 1
		var parent_path = gn.category % ("s" + str(stage) + "n")
		...
		if true:
			...
			i = 0
			stage = 1
			parent_path = gn.category % ("s" + str(stage) + "m")
		...
		if true:
			...
			i = 0
			stage = 2
			parent_path = gn.category % ("s" + str(stage) + "n")
...
func r_setup_setup():
	...
	if gv.open_upgrade_folder in ["s1n", "s2n"]:
...
func flash_reset_button():
	...
	var test = gv.PREFAB["flash"].instance()

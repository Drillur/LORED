...
func get_status_color(net: Array) -> Color:
	...
	if true:
		...
		if net[1].greater(net[0]):
			if gv.resource[key].less_equal(Big.new(gv.g[key].d.t).m(100)) and Big.new(net[1]).s(net[0]).less(Big.new(gv.g[key].d.t).m(0.01)):
				return status_colors[status.BAD]
	...
	if true:
		...
		if gv.resource[key].less_equal(Big.new(gv.g[key].d.t).m(2)) and Big.new(net[0]).s(net[1]).less(Big.new(gv.g[key].d.t).m(0.01)):
			return status_colors[status.FINE]
